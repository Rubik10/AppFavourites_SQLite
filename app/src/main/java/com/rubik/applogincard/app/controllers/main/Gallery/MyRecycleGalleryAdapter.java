...
import java.util.List;
             ...
             */
    public class MyRecycleGalleryAdapter extends RecyclerView.Adapter<MyRecycleGalleryAdapter.MyViewHolder> {
        private Context context;
        public List<Images> listImages;
        private SharedPreferences sharedPref;

        public MyRecycleGalleryAdapter (GalleryActivity galleryActivity, List<Images> listImages ) {
            this.context = galleryActivity.getApplicationContext();
            sharedPref = context.getSharedPreferences("image", Context.MODE_PRIVATE); //init shared pref
            this.listImages = listImages;
        }

            @Override
        public MyRecycleGalleryAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.gallery_card_layout, parent, false);
            return new MyViewHolder(itemView);
        }

            @Override
        public void onBindViewHolder(final MyRecycleGalleryAdapter.MyViewHolder holder, final int position) {
